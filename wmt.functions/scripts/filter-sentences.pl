...
while (defined(my $line1=<IN1>) && defined(my $line2=<IN2>)) {
	...
	$linesSkipped += 1;
	print STDERR "Skipping aligned sentence number $lineNumber, with respective word counts of $numWords1 and $numWords2\n";
    } elsif (($numWords1 / $numWords2 > 9) || ($numWords2 / $numWords1 > 9)) {
	$excessRatio += 1;
	print STDERR "Skipping aligned sentence number $lineNumber, with respective word counts of $numWords1 and $numWords2\n";
    } else {
	print OUT1 $line1;
...
print STDERR "Skipped $excessRatio of $lineNumber aligned sentences because of excessively unbalanced word counts.\n";
