...
define USAGE
	$(info  )
	$(info Usage:	make -f ${THIS.MAKEFILE} JOSHUA=/path/to/joshua joshua-extract-mbr-cand)
	$(info  )
	$(error )
endef
...
JOSHUA_EXTRACT_MBR_CAND_DIR ?= $(call USAGE)
JOSHUA_EXTRACT_MBR_THREADS ?= $(call USAGE)
...
joshua-extract-mbr-cand: ${JOSHUA_EXTRACT_MBR_CAND_DIR}/${JOSHUA_TRANSLATION_OUTPUT}
all: joshua-extract-mbr-cand

${JOSHUA_EXTRACT_MBR_CAND_DIR}/${JOSHUA_TRANSLATION_OUTPUT}: ${JOSHUA_TRANSLATION_DIR}/${JOSHUA_TRANSLATION_OUTPUT} | ${JOSHUA_EXTRACT_MBR_CAND_DIR}
	java -cp ${JOSHUA}/bin/joshua.jar -Dfile.encoding=utf8 \
		joshua.decoder.NbestMinRiskReranker $< $@ false 1 ${JOSHUA_EXTRACT_MBR_THREADS}


${JOSHUA_EXTRACT_MBR_CAND_DIR}:
	mkdir -p $@
